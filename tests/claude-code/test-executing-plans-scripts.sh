...
PLAN
  ...
  # --- task-done: node --test result is pass/fail, not duration_ms ---
  cat >"$repo/sample.test.js" <<'JS'
import { test } from 'node:test';
import assert from 'node:assert/strict';
test('ok', () => assert.equal(1, 1));
JS
  (cd "$repo" && git add sample.test.js && git "${git_id[@]}" commit -qm "sample test")
  out="$(cd "$repo" && "$EP_SCRIPTS/task-done" plan.md 2 "$head" -- node --test --test-reporter=tap sample.test.js)"
  rc=$?
  local ledger_line
